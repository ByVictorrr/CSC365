...
BEGIN
    ...
    SET c_in = STR_TO_DATE(CheckIn, '%d-%M-%y');
    SET c_out = STR_TO_DATE(CheckOut, '%d-%M-%y');
    ...
    IF (NEW.Room = Room AND new_in > new_out OR new_in <= c_in OR new_in > c_in AND new_in < c_out) THEN
